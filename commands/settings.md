...
## Step 6: Webhook / Feishu Configuration (Optional)

Do you want to send notifications to a webhook or Feishu/Lark?

Use AskUserQuestion with:
- question: "Enable remote notifications?"
- header: "🔗 Webhooks"
- multiSelect: false
- options:
  1. **No webhooks** - "Desktop notifications only (recommended)"
  2. **Feishu/Lark QR binding** - "Scan with Feishu/Lark mobile app and save app robot config"
  3. **Feishu/Lark custom webhook** - "Use an existing group custom bot webhook"
  4. **Slack** - "Send to Slack webhook (JSON format)"
  5. **Discord** - "Send to Discord webhook (embed format)"
  6. **Telegram** - "Send to Telegram bot (requires chat_id)"
  7. **Custom** - "Custom webhook endpoint (JSON)"

If the user selects **Feishu/Lark QR binding**, run:
...
if [ -x "${PLUGIN_ROOT}/bin/claude-notifications" ]; then
  "${PLUGIN_ROOT}/bin/claude-notifications" feishu bind
...
This opens a Feishu/Lark registration URL and prints it in the terminal. The user scans it with the Feishu/Lark mobile app. On success, the CLI writes `~/.claude/claude-notifications-go/config.json` with `"preset": "feishu_app"`.

Do not overwrite that generated Feishu config in Step 7. If the user selected QR binding, preserve the saved `notifications.webhook` and `notifications.feishu` fields, and only merge desktop sound/status settings.

If the user selects **Feishu/Lark custom webhook**, create a placeholder configuration with `"preset": "lark"` and tell them to paste their custom bot URL. If they use Feishu/Lark signing, store the secret through `"notifications.feishu.sign_secret_env"`, not inline.

If another webhook is enabled, create a placeholder configuration that the user can edit later.
...
- If user selected "Feishu/Lark QR binding": keep the config written by `claude-notifications feishu bind`; it sets `"enabled": true`, `"preset": "feishu_app"`, `"format": "json"`, and `notifications.feishu`
- If user selected "Feishu/Lark custom webhook": Set `"enabled": true`, `"preset": "lark"`, `"url": "<Feishu/Lark custom bot URL>"`
...
```json
{
  "notifications": {
    ...
    "webhook": {
      ...
      "preset": "<feishu_app|lark|slack|discord|telegram|custom - NEVER use 'none', use 'custom' if No webhooks>",
    ...
    "feishu": {
      "mode": "<app_registration for feishu_app, custom_webhook for lark custom bot, omit otherwise>",
      "app_id": "<set by feishu bind>",
      "app_secret": "<set by feishu bind>",
      "app_secret_env": "<optional env var name instead of app_secret>",
      "owner_open_id": "<set by feishu bind>",
      "receive_id_type": "<open_id|chat_id|user_id|email|union_id>",
      "receive_id": "<target id>",
      "sign_secret_env": "<optional custom webhook sign secret env var>"
    },
...
- **Feishu/Lark QR binding:** run `claude-notifications feishu bind`; it opens a scan URL and writes the config
- **Feishu/Lark custom webhook:** paste your custom bot URL; use `notifications.feishu.sign_secret_env` for signing
