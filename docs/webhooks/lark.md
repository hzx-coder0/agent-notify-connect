...
- `lark`: incoming custom bot webhook. Best for group chats.
- `feishu_app`: QR-created Feishu/Lark app robot. Best for one-click personal binding.
...
The command opens a Feishu/Lark registration URL and prints it in the terminal. Scan it with the Feishu/Lark mobile app. On success, the plugin writes:

```text
~/.claude/claude-notifications-go/config.json
```

Default target is the scanned user's `open_id`.

For group delivery, set a group `chat_id`:

```bash
claude-notifications feishu bind --receive-id-type chat_id --receive-id oc_xxx
```
...
`feishu bind` writes the local user config. Do not commit this file.

For stricter secret handling, move `app_secret` into an environment variable and edit the config:
...
```json
{
  "notifications": {
    "feishu": {
      "app_secret": "",
      "app_secret_env": "CLAUDE_NOTIFICATIONS_FEISHU_APP_SECRET"
    }
  }
}
```
