...
### Fixed
- **macOS: notification delivery no longer steals focus** - `ClaudeNotifier.app` now launches through `open -g`, so banner delivery does not briefly bring the notifier app to the foreground before any notification click ([#82](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/82))
- **Plugin setup commands stay out of model context** - setup/config slash commands remain manually available from the slash menu while opting out of model invocation metadata to reduce prompt overhead after setup ([#83](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/83))
...
- **macOS: notification permission remediation is more robust** — native notifier permission problems now surface cleaner remediation/fallback behavior, with cross-platform tests updated to keep CI stable ([#64](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/64))

## [1.36.2] - 2026-04-02

### Fixed
- **GNOME/Wayland: notification click no longer leaves a long loading cursor** — on Ubuntu 24.04 GNOME/Wayland, notification clicks now use a hidden `claude-notifications.desktop` entry with `StartupNotify=false`, avoiding an unconsumed activation token while preserving click-to-focus behavior and the earlier Nautilus fallback fix ([#61](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/61))

## [1.36.1] - 2026-04-02

### Fixed
- **macOS: ClaudeNotifier notifications use native app attribution again** — the notifier now launches through LaunchServices instead of executing the app binary directly, preventing `UNUserNotificationCenter` from degrading into Script Editor attribution on recent macOS releases ([#59](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/59))
...
### Added
- **Team mode support** — smart notification handling for Claude Code teams ([#48](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/48)). New `teamMode` config option with three modes:
...
### Fixed
- **Windows: remove redundant `sh` prefix from hook commands** — Claude Code already spawns a shell for hooks; the extra `sh` was misinterpreted as a script filename on some Windows environments, causing "cannot execute binary file" ([#35](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/35))
...
### Added
- **Ghostty click-to-focus** — clicking a notification focuses the correct Ghostty window via AXDocument (OSC 7 CWD file:// URL) ([#34](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/34))
...
### Added
- **Zellij click-to-focus support** — clicking a notification switches to the correct zellij tab ([#33](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/33))
...
### Added
- **Window-specific click-to-focus** — clicking a notification now focuses the exact project window instead of just activating the app ([#31](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/31))
...
### Fixed
- **Config survives plugin updates** — config.json moved to `~/.claude/claude-notifications-go/config.json` outside the plugin cache, so settings are no longer lost when bootstrap.sh clears the cache during updates ([#30](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/30))
...
### Added
- **ntfy.sh webhook documentation** — custom webhook example for push notifications via ntfy.sh ([#29](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/29))
...
### Added
- **Terminal bell for tab indicators** 🔔 ([#28](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/28), thanks @retr0h!)
...
- **`list-sounds` CLI utility** — lists all available notification sound files ([#23](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/23))
...
### Added
- **GNOME Terminal detection** — detect GNOME Terminal via `GNOME_TERMINAL_SCREEN` / `GNOME_TERMINAL_SERVICE` env vars for click-to-focus support on Linux ([#22](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/22), thanks @ductrantrong)

### Fixed
- **Windows checksum verification** — strip leading backslash from `sha256sum` output on Windows (MSYS2/Git Bash/Cygwin) which caused checksum mismatch ([#26](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/26))
...
### Fixed
- **install.sh** — `install_gnome_activate_window_extension()` now returns failure code when GNOME shell/extensions not found or extension couldn't be enabled ([#19](https://github.com/hzx-coder0/claude-codex-notifications-go/pull/19))

## [1.14.0] - 2026-01-16

### Added
- **Per-status notification control** 🎛️ ([#16](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/16))
...
### Fixed
- **Installer now downloads utility binaries** ([#14](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/14))
...
### Fixed
- **Ghost notifications after 60 seconds** 👻 ([#11](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/11))
...
### Fixed
- **Webhook notifications never sent** ([#6](https://github.com/hzx-coder0/claude-codex-notifications-go/issues/6))
...
[1.0.2]: https://github.com/hzx-coder0/claude-codex-notifications-go/compare/v1.0.1...v1.0.2
[1.0.1]: https://github.com/hzx-coder0/claude-codex-notifications-go/compare/v1.0.0...v1.0.1
[1.0.0]: https://github.com/hzx-coder0/claude-codex-notifications-go/releases/tag/v1.0.0
