...
func TestHandler_WebhookUsesFullAssistantMessage(t *testing.T) {
	cfg := &config.Config{
		Notifications: config.NotificationsConfig{
			Desktop: config.DesktopConfig{Enabled: true},
			Webhook: config.WebhookConfig{Enabled: true},
	...
	handler, mockNotif, mockWH := newTestHandler(t, cfg)
	fullReply := strings.Repeat("完整模型回复。", 120)
	transcriptPath := createTempTranscript(t, []jsonl.Message{
		{
			Type:      "user",
			Timestamp: "2025-01-01T12:00:00Z",
			Message:   jsonl.MessageContent{ContentString: "do it"},
		},
		{
			Type:      "assistant",
			Timestamp: "2025-01-01T12:00:01Z",
	...
	if len(mockNotif.calls) != 1 {
		t.Fatalf("desktop calls = %d, want 1", len(mockNotif.calls))
	}
	if strings.Contains(mockNotif.calls[0].message, fullReply) {
		t.Fatalf("desktop message should stay summarized, got full reply")
	}
	if len(mockWH.calls) != 1 {
		t.Fatalf("webhook calls = %d, want 1", len(mockWH.calls))
	}
	if mockWH.calls[0].rawBody != fullReply {
		t.Fatalf("webhook raw body length = %d, want full length %d", len([]rune(mockWH.calls[0].rawBody)), len([]rune(fullReply)))
	}
	if !strings.Contains(mockWH.calls[0].message, fullReply) {
		t.Fatalf("webhook message should include full reply")
	}
}
