...
import (
	...
	"github.com/hzx-coder0/agent-notify-connect/internal/benchmark"
	...
	"github.com/hzx-coder0/agent-notify-connect/internal/notification"
	...
	"github.com/hzx-coder0/agent-notify-connect/internal/summary"
...
func (h *Handler) HandleHook(hookEvent string, input io.Reader) error {
	...
	webhookBody := h.generateWebhookBody(status, parsedMessages)
	...
	h.sendNotificationsWithWebhookBody(status, body, actions, webhookBody, hookData.SessionID, hookData.CWD)
...
func (h *Handler) generateWebhookBody(status analyzer.Status, messages []jsonl.Message) string {
	switch status {
	case analyzer.StatusSessionLimitReached, analyzer.StatusAPIError, analyzer.StatusAPIErrorOverloaded:
		return ""
	}

	text := strings.TrimSpace(lastAssistantText(messages))
	if text == "" {
		return ""
	}
	return text
...
func lastAssistantText(messages []jsonl.Message) string {
	for i := len(messages) - 1; i >= 0; i-- {
		if messages[i].Type != "assistant" {
			continue
		}
		var parts []string
		for _, content := range messages[i].Message.Content {
...
func joinMessageParts(body, actions string) string {
	return notification.JoinMessageParts(body, actions)
}
...
func (h *Handler) sendNotifications(status analyzer.Status, body, actions, sessionID, cwd string) {
	h.sendNotificationsWithWebhookBody(status, body, actions, "", sessionID, cwd)
}

func (h *Handler) sendNotificationsWithWebhookBody(status analyzer.Status, body, actions, webhookBody, sessionID, cwd string) {
	service := notification.New(h.cfg, h.notifierSvc, h.webhookSvc)
	service.OnDesktopError = h.maybeEmitDesktopPermissionGuidance
