...
func TestLoadConfigWithUTF8BOM(t *testing.T) {
	...
	configJSON := append([]byte{0xEF, 0xBB, 0xBF}, []byte(`{
		"notifications": {
			"desktop": {
				"enabled": false
			},
	...
	err := os.WriteFile(configPath, configJSON, 0o644)
	require.NoError(t, err)
...
func TestValidateConfig_FeishuAppRequiresReceiveID(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Notifications.Webhook.Enabled = true
	cfg.Notifications.Webhook.Preset = "feishu_app"
	cfg.Notifications.Feishu = FeishuConfig{
	...
	err := cfg.Validate()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "receive_id")
}

func TestApplyDefaults_FeishuAppUsesOwnerOpenID(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Notifications.Webhook.Preset = "feishu_app"
	cfg.Notifications.Feishu = FeishuConfig{
		Mode:        "app_registration",
	...
	assert.Equal(t, "feishu", cfg.Notifications.Feishu.Platform)
	assert.Equal(t, "open_id", cfg.Notifications.Feishu.ReceiveIDType)
	assert.Equal(t, "ou_owner", cfg.Notifications.Feishu.ReceiveID)
}
