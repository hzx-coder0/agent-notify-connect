...
func TestSenderLarkWebhookAddsSignature(t *testing.T) {
	var receivedPayload map[string]interface{}
	...
	cfg.Notifications.Feishu.SignSecret = "secret"
	sender := New(cfg)
	...
	if receivedPayload["timestamp"] == "" {
		t.Fatalf("expected timestamp in signed lark payload: %#v", receivedPayload)
	}
	if receivedPayload["sign"] == "" {
		t.Fatalf("expected sign in signed lark payload: %#v", receivedPayload)
	}
	if receivedPayload["msg_type"] != "interactive" {
		t.Fatalf("expected lark interactive payload, got %#v", receivedPayload["msg_type"])
	}
}

func TestLarkFormatterUsesRawBodyAsCardContent(t *testing.T) {
	fullReply := strings.Repeat("完整模型回复。", 120)
	payload, err := (&LarkFormatter{}).Format(SendContext{
		Status:    analyzer.StatusTaskComplete,
		Message:   "[session repo] Completed",
		RawBody:   fullReply,
		SessionID: "session-123",
		Folder:    "repo",
		GitBranch: "main",
	}, config.StatusInfo{Title: "Completed"})
	if err != nil {
		t.Fatalf("Format() error = %v", err)
	}

	root := payload.(map[string]interface{})
	card := root["card"].(map[string]interface{})
	elements := card["elements"].([]map[string]interface{})
	text := elements[0]["text"].(map[string]interface{})
	if text["content"] != fullReply {
		t.Fatalf("card content length = %d, want full length %d", len([]rune(text["content"].(string))), len([]rune(fullReply)))
	}
}
