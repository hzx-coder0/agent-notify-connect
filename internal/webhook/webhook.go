...
func (s *Sender) sendWithRetryAndCircuitBreaker(requestID string, sendCtx SendContext) error {
	...
	if webhookCfg.Preset == "feishu_app" {
		return s.sendFeishuApp(requestID, sendCtx, payload)
	}
...
func (s *Sender) sendFeishuApp(requestID string, sendCtx SendContext, payload []byte) error {
	var card map[string]interface{}
	if err := json.Unmarshal(payload, &card); err != nil {
		return fmt.Errorf("failed to decode feishu card payload: %w", err)
	}

	sendFn := func(ctx context.Context) error {
		logging.Debug("[%s] Sending Feishu app message for status=%s", requestID, sendCtx.Status)
		return feishu.NewAppSender(s.cfg.Notifications.Feishu).Send(ctx, card["card"])
	}

	if s.circuitBreaker != nil {
		return s.circuitBreaker.Execute(s.ctx, func() error {
			return s.retry.Do(s.ctx, sendFn)
		})
	}
	return s.retry.Do(s.ctx, sendFn)
...
func (s *Sender) buildPayload(runtimeCtx *runtimeContext) ([]byte, string, error) {
	...
	if formatter, ok := s.formatters[webhookCfg.Preset]; ok {
		...
		if webhookCfg.Preset == "lark" {
			if err := s.applyLarkSignature(payload, runtimeCtx.now); err != nil {
				return nil, "", err
			}
		}
...
func (s *Sender) applyLarkSignature(payload interface{}, now time.Time) error {
	secret := s.cfg.Notifications.Feishu.ResolveSignSecret()
	if secret == "" {
		return nil
	}
	...
	if !ok {
		return fmt.Errorf("lark signature requires JSON object payload")
	}

	timestamp := now.Unix()
	payloadMap["timestamp"] = fmt.Sprintf("%d", timestamp)
	payloadMap["sign"] = feishu.SignCustomWebhook(timestamp, secret)
	return nil
}
