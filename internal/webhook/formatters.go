...
func (f *LarkFormatter) Format(ctx SendContext, statusInfo config.StatusInfo) (interface{}, error) {
	content := strings.TrimSpace(ctx.RawBody)
	if content == "" {
		content = ctx.Message
	}
	sessionLine := fmt.Sprintf("Session: %s", ctx.SessionID)
	if ctx.Folder != "" {
		sessionLine = fmt.Sprintf("%s | Folder: %s", sessionLine, ctx.Folder)
	}
	if ctx.GitBranch != "" {
		sessionLine = fmt.Sprintf("%s | Branch: %s", sessionLine, ctx.GitBranch)
	}
