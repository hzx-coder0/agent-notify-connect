...
INSTALLER=notification-installer
GUI_INSTALLER=notification-installer-gui
BINARY_PATH=bin/$(BINARY)
SOUND_PREVIEW_PATH=bin/$(SOUND_PREVIEW)
LIST_SOUNDS_PATH=bin/$(LIST_SOUNDS)
INSTALLER_PATH=bin/$(INSTALLER)
GUI_INSTALLER_PATH=bin/$(GUI_INSTALLER)
...
build: ## Build the binaries (development mode with debug symbols)
	@echo "Building $(BINARY), $(SOUND_PREVIEW), $(LIST_SOUNDS), $(INSTALLER) and $(GUI_INSTALLER) (development mode)..."
	...
	@go build -o $(INSTALLER_PATH) ./cmd/notification-installer
	@go build -o $(GUI_INSTALLER_PATH) ./cmd/notification-installer-gui
...
build-all: ## Build optimized binaries for all platforms
	...
	@echo "Building notification-installer..."
	@GOOS=windows GOARCH=amd64 go build $(RELEASE_FLAGS) -o dist/$(INSTALLER)-windows-amd64.exe ./cmd/notification-installer
	@GOOS=windows GOARCH=amd64 go build $(RELEASE_FLAGS) -o dist/$(GUI_INSTALLER)-windows-amd64.exe ./cmd/notification-installer-gui
